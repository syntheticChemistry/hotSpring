...
#
# Post-primordial (Wave 49+): All NUCLEUS primal binaries come exclusively
# from plasmidBin. No target/release/ fallback. No cargo build.
#
...
echo ">>> Installing toadStool diesel engine (plasmidBin-only)..."

if [ ! -d "$PLASMIDBIN" ] || [ ! -x "$PLASMIDBIN/fetch.sh" ]; then
    echo "FATAL: plasmidBin not found at $PLASMIDBIN" >&2
    echo "  Post-primordial mandate: all NUCLEUS primals come from plasmidBin." >&2
    echo "  Run: git clone <plasmidBin-repo> $PLASMIDBIN" >&2
    echo "  Or:  tools/fetch_primals.sh from primalSpring" >&2
    exit 1
fi
