# CompChem GuideStone v1.7.0

**Self-verifying baseline for enhanced sampling molecular dynamics — with automated artifact detection.**
...
**Critical fix:** v1.6.1 enzyme-bound simulations had a substrate dissociation artifact — the xylose monomer escaped the active site during production metadynamics, making the "enzyme-bound" FEL effectively identical to the free-sugar FEL. This was identified by domain expert review (Alistaire).

**v1.7.0 fixes:**
- Added `UPPER_WALLS` flat-bottom restraint on substrate–enzyme COM distance (1.2 nm threshold, KAPPA=500 kJ/mol/nm²)
- Re-ran all enzyme-bound metadynamics with binding monitor (d_bind column in COLVAR)
- **Automated detection pipeline** — three new checks prevent this class of artifact from shipping undetected:
  1. **Cross-landscape check:** Free vs bound FES RMSD must exceed 5 kJ/mol (identical = substrate escaped)
  2. **Binding distance monitor:** max(d_bind) > 2.0 nm → FAIL
  3. **KS distribution test:** If theta distributions are indistinguishable at α=0.05 → suspicious
- Improved figures: (θ,φ) Mercator projections with IUPAC Stoddart labels, difference maps
...
| 05 | Enzyme-bound GH10 1D | WTMetaD theta + UPPER_WALLS | 10 ns | Barrier lowering (restrained, d<1.2nm) |
| 06 | Enzyme-bound GH10 2D | WTMetaD qx,qy + UPPER_WALLS | 20 ns | Conformational selection (restrained) |
...
## Derivation Anchoring (Zero Magic Numbers)

This artifact implements the **Derivation Anchoring Standard** v1.0
(`ecoPrimals/infra/wateringHole/DERIVATION_ANCHORING_STANDARD.md`).

Every numeric threshold that drives a pass/fail decision is formally derived in
`derivations/threshold_calibration.toml` — the single source of truth. The
validator loads these values at runtime, and Phase 0 (Constant Self-Consistency)
enforces that compiled fallbacks match the derivation file exactly.

```
derivations/threshold_calibration.toml   ← 23 anchored constants
       │
       │  GuideStoneConstants::load()
       ▼
  nest-validate uses derived values
       │
       │  Phase 0 verifies
       ▼
  190/190 checks PASS (including self-consistency)
```

Each constant traces through a 5-layer empirical chain: numerical precision →
reproducibility floor → measured signal → per-basin decomposition → literature
calibration. No untethered numbers. No comments-as-documentation. Structural
enforcement.
