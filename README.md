...
| **Sovereign GPU** (toadStool + coralReef compile) | ✅ **Tier 1 Validated** | RTX 5060 dispatch live (QCD/HMC/MD validated on SM120). **Dual Titan V** (identical GV100, VBIOS-matched, IOMMU-isolated — Exp 205). Warm-catch pipeline **pure Rust**. **Sovereignty Tier Model** (Exp 210): Tier 0 cold → Tier 1 warm infra (**validated**) → Tier 2 warm compute (blocked — GPC power domain) → Tier 3 full sovereign. PTOP parser fixed, CE runlist discovered, anchor-fd adoption bridges ember→dispatch gap (Exp 209). **Warm Keepalive** (Exp 208): 183ms pipeline, fd store e2e, falcon preservation, 76× faster than cold. |
...
*210 experiments, 596 / 1,045 lib tests (IPC-first default / barracuda-local), 167 binaries, 128 WGSL shaders, ~$0.30 total science cost.
