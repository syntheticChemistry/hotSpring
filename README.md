...
hotSpring has evolved from the prokaryotic era of separate binaries into an eukaryotic UniBin — a single `hotspring_unibin` binary consolidating certification (L0–L6 guideStone organelle), validation scenarios (17 default / 23 with `barracuda-local`), and status reporting. Reference: primalSpring v0.9.25 interstadial eukaryotic wave.
...
│
├── experiments/                        # 198 experiment journals (fossil record); 001-190 archived under experiments/archive/
│   ├── archive/                        # experiments 001-190 (archived journals + FOSSIL_RECORD summaries)
│   └── 191-198: active experiments (toadStool PBDMA, HW validation, PLX keepalive, cold/warm boot, driver lab, warm swap, sovereign init, boot pipeline)
│
