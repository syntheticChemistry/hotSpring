...
| **Sovereign GPU** (toadStool + coralReef compile) | ✅ **Dual Titan V sovereign** | RTX 5060 dispatch live (QCD/HMC/MD validated on SM120). **Dual Titan V** (identical GV100, VBIOS-matched, IOMMU-isolated — Exp 205). Warm-catch pipeline **pure Rust**. SLM pool allocation (2 MiB), AMD scratch/local f64 PASS, Ember gate + survivability hardening complete, **Diesel Engine** validated, **PowerSafetyProfile** (K80 fire post-mortem). **Sovereign Boot Abstraction** (Exp 207): `SovereignBootState` enum, `WarmKeepalive` facade, `sovereign.profile` RPC, twin-card profiling. **Hardware line codified**: cold boot = vendor wall. |
...
│
├── experiments/                        # 207 experiment journals (fossil record); 001-190 archived under experiments/archive/
│   ├── archive/                        # experiments 001-190 (archived journals + FOSSIL_RECORD summaries)
│   └── 191-207: active experiments (toadStool PBDMA, HW validation, PLX keepalive, cold/warm boot, driver lab, warm swap, diesel engine, power safety, CG sweep, surface rewire, warm/cold convergence, VBIOS live validation, dual Titan V, Falcon ACR DMA, sovereign boot abstraction + profiling)
│
