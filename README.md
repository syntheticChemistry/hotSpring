...
> **New (Exp 097-100):** **Full silicon characterization pipeline.** Four benchmark binaries: `bench_silicon_budget` (theoretical peak per GPU per unit — 15+ GPU models including A100/H100/MI250X reference), `bench_silicon_saturation` (actual peak: FMA, bandwidth, cache hierarchy, TMU, LDS, atomics), `bench_silicon_composition` (compound multi-unit throughput), and evolved `bench_qcd_silicon` v2 (14 kernels: quenched + dynamical + observables, FP32 + DF64, 4^4 → 32^4 production scale). **GPU RHMC infrastructure:** `multi_shift_zeta_f64.wgsl` shader + `gpu_rhmc.rs` module. **Silicon characterization at scale (Exp 100):** HPC silicon waste analysis (A100 0.6%, H100 0.4% utilization by QUDA-style codes), decommissioned HBM2 fleet strategy (Titan V + MI50 + Tesla P80), CERN collaboration path. 100 experiments. 4,065+ tests pass.
>
