...
| **Ember Swap Pipeline Evolution** (Exp 074) | ✅ Complete | **D-state resilient sysfs** — process-isolated watchdog (10s timeout, child-process fork for risky kernel writes). **IOMMU group peer release** for native driver swap (audio device unbind). **EmberClient retry** (3× backoff for EAGAIN/EINTR). **DRM isolation auto-generation** from config at startup. **iommufd loaded at boot**. **nouveau ↔ vfio round-trip proven** on Titan V. Hardware: **2× Titan V + RTX 5060** (MI50 swapped out for second Titan). 74 experiments. |
...
│   └── handoffs/                       # 14 active + 90 archived unidirectional handoff documents
│
├── benchmarks/
