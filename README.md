...
| **Sovereign GPU** (coralReef) | ✅ Multi-gen sovereign | RTX 5060 dispatch live (QCD/HMC/MD validated on SM120), **K80 warm NOP dispatch wired** (PCIe link requires physical power cycle — PLX D3cold), **Titan V: HBM2 warm-handoff proven** (`reset_method=none` preserves 12GB HBM2 across driver swap), **Volta ACR solver skip** (sovereign_stages 4s completion), **FECS path: benchScale VM + nvidia-470**, **SLM pool allocation** (2 MiB), AMD scratch/local f64 PASS, Ember gate + survivability hardening complete, **Diesel Engine** (hierarchical glowplug/ember) validated |
...
- **1,025 tests** (lib), **155 binaries**, **64 validation suites** (64/64 pass via `validate_all`; 84 individual `validate_*` binaries + `hotspring_guidestone`), **128 WGSL shaders** (all AGPL-3.0-only),
...
```bash
cd barracuda
cargo test               # 1,025 tests (lib), 6 ignored (~120s; spectral tests upstream)
...
│
├── barracuda/                          # BarraCuda Rust crate (1,025 tests, 155 binaries, 128 WGSL shaders)
...
*189 experiments, 1,025 tests, 155 binaries, 128 WGSL shaders, ~$0.30 total science cost.
