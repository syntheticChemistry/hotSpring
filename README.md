...
| **Sovereign GPU** (toadStool + coralReef compile) | ✅ **Warm Keepalive PROVEN** | RTX 5060 dispatch live (QCD/HMC/MD validated on SM120). **Dual Titan V** (identical GV100, VBIOS-matched, IOMMU-isolated — Exp 205). Warm-catch pipeline **pure Rust**. SLM pool allocation (2 MiB), AMD scratch/local f64 PASS, Ember gate + survivability hardening complete, **Diesel Engine** validated, **PowerSafetyProfile** (K80 fire post-mortem). **Sovereign Boot Abstraction** (Exp 207): `SovereignBootState` enum, `WarmKeepalive` facade, `sovereign.profile` RPC, twin-card profiling. **Hardware line codified**: cold boot = vendor wall. **Warm Keepalive** (Exp 208): fd store end-to-end, GPUs warm across daemon restart, cold 70× faster, warm pipeline 3.9s. |
