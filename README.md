...
> **150+ experiments** | **500+ quantitative checks** | **~$0.30 total science cost** | **870 lib tests, 139 binaries, 99 WGSL shaders** | **guideStone artifact: 59/59 checks x 5 substrates (x86_64 + aarch64)** | **OCI container image + Windows/macOS launchers** | **NVIDIA GPFIFO pipeline OPERATIONAL on RTX 3090** | **AMD scratch/local memory OPERATIONAL on RX 6950 XT** | **AMD sovereign compiler: 24/24 QCD shaders compile to native GFX ISA** | **Ember Survivability Hardening COMPLETE — system survives repeated GPU faults with zero lockups**
>
> **Ember Survivability Hardening (Exp 140-151, 2026-04-07):** Three-phase hardening eliminated all known lockup vectors. **Phase 1**: Fork-isolated every MMIO path (preflight, low-level read/write/batch, post_swap_quiesce), removed all parent-side sysfs writes from recovery paths (zero-I/O in fault handlers), replaced `std::process::exit` with `abort()` (no cleanup stalls), removed `sync_all()` from trace. **Phase 2**: Guarded sysfs reads/writes with timeouts, fire-and-forget tracing in fault paths. **Phase 3**: Glowplug resurrection now includes GPU warm cycle (nouveau bind/unbind), `ember.warm_cycle` RPC for live warm cycling, FdVault integrated for zero-downtime resurrection (backup VFIO fds keep GPU binding alive through ember death). **Validated**: 8 consecutive exp145 runs — zero lockups, ember stays alive, faults reported as clean errors. **Fleet:** Titan V (GV100) + 2× Tesla K80 (GK210) + RTX 5070.
>
> **SEC2 ACR Boot Investigation (Exp 141-151):** SEC2 falcon starts and executes BL code but does not achieve HS mode (`SCTL=0x3000`). Instance block + virtual DMA wired (bind_stat polling completes). IMEM survives STARTCPU (PIO content preserved). BROM registers readable. Root cause narrowing: VBIOS DEVINIT initially suspected (Exp 141-142), then **contradicted** — ACR fails even on BIOS-POSTed GPU. Crash vector hunt (Exp 150) isolated PRAMIN as the lockup trigger. Cold VRAM (`0xbad0ac0X`) now detected and reported gracefully.
>
