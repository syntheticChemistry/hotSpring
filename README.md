...
>
> **Ember Survivability Hardening (Exp 140-151, 2026-04-07):** Three-phase hardening eliminated all known lockup vectors. Fork-isolated MMIO, zero-I/O recovery, FdVault checkpoint/restore, GPU warm cycle resurrection. **Validated**: 8 consecutive fault runs — zero lockups. **Fleet:** 2× Titan V (GV100) + RTX 5060 (Blackwell). K80 replacement on order (original caught fire, Exp 199 → pulled, replaced by second Titan V; new K80 incoming for cross-generational sovereign validation).
>
...
| **Sovereign GPU** (toadStool + coralReef compile) | ✅ **Tier 1 Validated** | **Evolution: agnostic → atheistic (infra) → atheistic (compute) → deistic.** RTX 5060 dispatch live via DRM (12/12 QCD/HMC/MD on SM120). **Dual Titan V** VFIO sovereign Tier 1 proven (Exp 210): PFIFO, PBDMA, FECS, topology, CE channel — all working; GPC power domain is the Tier 2 wall. **Next unlock**: PMU mailbox protocol (Exp 211) to ungate GPCs from alive PMU falcon. **K80 incoming** — unsigned Kepler falcons may reach Tier 2 before Volta (no ACR barrier). coralReef: SM35 + SM70 + SM120 + GFX10.3. **183ms warm pipeline** (Exp 208). See `SILICON_DEISM.md` for the full abstraction elimination thesis. |
...
Full validation table (211 rows) with per-experiment details: [`EXPERIMENT_INDEX.md`](EXPERIMENT_INDEX.md)
...
│
├── experiments/                        # 213 experiment journals (fossil record); 001-190 archived under experiments/archive/
│   ├── archive/                        # experiments 001-190 (archived journals + FOSSIL_RECORD summaries)
│   └── 191-212: active experiments (toadStool PBDMA, HW validation, PLX keepalive, cold/warm boot, driver lab, warm swap, diesel engine, power safety, CG sweep, surface rewire, warm/cold convergence, VBIOS live validation, dual Titan V, Falcon ACR DMA, sovereign boot abstraction + profiling, reboot-efficient sovereign evolution, VFIO dispatch bridge, GPC boundary, PMU mailbox Tier 2, sovereign driver rotation, sovereignty consolidation + revalidation)
│
...
*213 experiments, 596 / 1,045 lib tests (IPC-first default / barracuda-local), 167 binaries, 128 WGSL shaders, ~$0.30 total science cost.
...
RHMC eliminates hand-tuned parameters. Chuna 44/44 checks pass. **Fleet: 2× Titan V (GV100) + RTX 5060 (Blackwell) + K80 incoming (GK210)** —
...
(QMD v5.0, SM120). Sovereignty tier model codified (Exp 210).
**Vendor-atheistic target**: not just agnostic to vendor, but independent of
vendor entirely — solving sovereign compute across Kepler, Volta, Blackwell,
and AMD RDNA2. K80 replacement on order for cross-generational validation.
Three-tier validation: Python validates Rust. Rust validates NUCLEUS.
