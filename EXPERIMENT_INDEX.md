...
| **PMU Mailbox Tier 2 Investigation** (Exp 211) | ⏳ Framed | **Path to Tier 2 sovereign compute** — PMU falcon at 0x10A000+ alive post-unbind. Investigation plan: (A) PMU liveness probe, (B) enumerate known PMU commands from nouveau/envytools, (C) PMU command injection (PG_CTRL to ungate GPC domain), (D) fallback PGOB direct override, (E) fallback kernel patch to nouveau `gv100_gr_fini()`. **Success criteria**: GPC_ENABLES at 0x41A004 returns non-fault value. **K80 angle**: unsigned Kepler falcons (no ACR barrier) may provide first Tier 2 before Volta. See `experiments/211_PMU_MAILBOX_TIER2_INVESTIGATION.md` |
