...
> Updated May 25, 2026. This is the authoritative ledger of all
...
**228 experiments** | **500+ quantitative checks** | **~$0.30 total science cost** | **AGPL-3.0-only**
...
| **Catalyst RM Channel** (Exp 229) | IN PROGRESS | **RM compute channel before warm swap to overcome FECS ACR.** Extended `rm_trigger --channel` to 16-step Volta RM channel recipe: root → device → subdevice → GR_GET_INFO → VA space → USERD/GPFIFO/notifier memory → TSG → ctx share → GPFIFO channel → compute → BIND → SCHEDULE → work submit token. `RmChannelEvidence` captured (channel_id, work_submit_token). PCCSR scan in catalyst_capture verifies RM channel ACTIVE before swap. Phase B: new sovereign channel post-swap (hypothesis: FECS primed). Phase A fallback: adopt RM channel hardware layout. See `experiments/229_CATALYST_RM_CHANNEL.md` |
