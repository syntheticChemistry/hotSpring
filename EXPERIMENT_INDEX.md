...
**218 experiments** | **500+ quantitative checks** | **~$0.30 total science cost** | **AGPL-3.0-only**
...
| **nvidia-470 nvsov Dual-Load Injection** (Exp 218) | 🔄 In Progress | **nvidia-470 co-load isolation SOLVED.** 4 blockers resolved: ksymtab stripping (objcopy), co-load isolation NOPs (5 targets: nv_cap_init, nv_cap_drv_init, nv_procfs_init, nvidia_register_module, nv_cap_procfs_init), ret0 at offset+5 (ftrace preamble bypass), PC32/PLT32 relocation normalization (kernel 6.17). Module loads alongside nvidia-580 without oops. Reboot needed to clear zombie module from test session, then full `sovereign.warm_handoff` pipeline test. See `experiments/218_NVIDIA470_NVSOV_DUAL_LOAD.md` |
