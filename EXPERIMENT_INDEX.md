...
| **TOTAL** | **64/64 validation suites** | **985 tests (lib)**, 166 binaries (84 `validate_*` + `hotspring_guidestone`), 128 WGSL shaders. Zero clippy, zero unsafe, zero `dyn` dispatch (prod), AGPL-3.0-only. `deny.toml` enforced (ecoBin C-dep bans). `#[expect(lint, reason)]` in all production code. **Science ladder:** Quenched → Gradient Flow → Integrators → N_f=4 Infra → Chuna 44/44 → N_f=2 → N_f=2+1 → Self-tuning → Silicon saturation → 16⁴+ production → **Firmware Boundary → NOP Dispatch → SovereignInit Pipeline (pure Rust, zero nouveau) → NUCLEUS Composition Validation → Composition Evolution → Primal Composition Proof → Level 5 Primal Proof → guideStone (hotspring_guidestone: primalspring::composition API, bare + NUCLEUS)**. 168+ experiments. Experiments 001-143 archived to `experiments/archive/` (completed physics validation + sovereign GPU + ember hardening, absorbed into baseCamp and coralReef). |
