...
| **Catalyst Driver Pattern** (Exp 219) | 🔧 Infra Complete | **Catalyst driver pattern for TPC sovereignty.** Treats nvidia-470 as chemical catalyst: load → init GPU compute pipeline → capture BAR0 golden state → remove catalyst → replay on future boots. `nvidia_catalyst_handoff` patch set (selective un-NOP: `nv_cap_init`/`nv_cap_drv_init` removed), catalyst capture step in warm handoff pipeline, `Bar0Snapshot::to_catalyst_replay()`, `Bar0Diff::to_replay_sequence()`, 3-layer preservation (recipe TOML + frozen .ko + product JSONs), `engine_init_path` wired in ember handler, `sovereign.catalyst_diff` twin-card RPC, `sovereign.catalyst_boot` catalyst-free boot RPC. 3 new RPCs (24 total). Awaiting hardware execution. See `experiments/219_CATALYST_DRIVER_PATTERN.md` |
