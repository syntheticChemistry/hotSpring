...
**Date**: February 26, 2026 (inception); updated May 13, 2026 (v0.6.32 — guideStone Level 6 CERTIFIED (NUCLEUS Deployment Validation), primalSpring v0.9.25 post-interstadial Tier 4 IPC-first. 592 (default) / 1,041 (barracuda-local) lib tests, 167 binaries, 64/64 validation suites, 128 WGSL shaders, 17/20 validation scenarios, 25/25 papers reproduced (CPU), 20/25 GPU. Deep debt resolution complete — zero TODO/FIXME/HACK, zero production mocks, zero library unsafe, zero C deps (default), zero clippy. 7 deploy graphs, provenance trio IPC, skunkBat audit IPC wired, foundation Thread 2 seeded. All IPC uses `call_by_capability()`. Sovereign GPU validation niche wired. Node atomic scenario added.  
...
|--------|-------|
| Crate | v0.6.32 |
| Lib tests | **592** pass (default) / **1,041** pass (barracuda-local), 6 GPU/heavy-ignored |
