...
**Date**: February 26, 2026 (inception); updated May 26, 2026 (v0.6.32 — guideStone Level 6 CERTIFIED (NUCLEUS Deployment Validation), primalSpring v0.9.27 post-interstadial Tier 4 IPC-first. 720 (cylinder) / 596 (default) / 1,045 (barracuda-local) lib tests, 155 binaries, 65/65 validation suites, 154 WGSL shaders, 18/24 validation scenarios, 234 experiments, 25/25 papers reproduced (CPU), 20/25 GPU. 7 deploy graphs, provenance trio IPC, skunkBat audit IPC wired. Sovereignty Tier Model — Tier 1 warm infrastructure validated (Exp 210), TPC wall confirmed (Exp 217), Catalyst Driver Pattern infra complete (Exp 219), UEFI Model mapped (Exp 221), Exp 225 vfio-pci reset-on-release identified → diesel engine FLR-first fix applied. 24 RPC methods. Fleet: 2× Titan V (GV100) + RTX 5060 (Blackwell). biomeGate NUCLEUS launched — sole tenant, 9-primal composition live via plasmidBin. **CAZyme FEL (Exp 220)**: pseudoSpore v1.7.0 / lithoSpore v2.3.0 — 8-module GuideStone-grade data chassis, 12/12 audit PASS, Domain Profiles, unified liveSpore.json schema, PLUMED-NEST validation aggregate, exploration roadmap, handoff-ready.  
...
## Codebase Health (May 16, 2026)

| Metric | Value |
|--------|-------|
| Crate | v0.6.32 |
| Lib tests | **596** pass (default) / **1,045** pass (barracuda-local), 6 GPU/heavy-ignored |
...
| Eukaryotic UniBin | `hotspring_unibin` — L0-L6 certification + 17 validation scenarios via `ScenarioRegistry` (20 with barracuda-local) |
