...
**Status:** v0.6.32, experiments 001-165, 870 lib tests, 39/39 validation suites, 143 binaries, 128 WGSL shaders. **NVIDIA GPFIFO pipeline OPERATIONAL on RTX 3090.** **AMD scratch/local memory OPERATIONAL on RX 6950 XT.** AMD sovereign compiler: 24/24 QCD shaders → native GFX ISA. **Firmware Boundary → SovereignInit Pipeline COMPLETE** — pure Rust cold-to-compute GPU init replaces nouveau subsystem by subsystem. **NUCLEUS Composition Validation COMPLETE** — 4 atomic validation binaries (Tower/Node/Nest/FullNucleus), IPC-wired primal health probes, `composition.*` JSON-RPC endpoints, MCP tool surface, ecoBin packaging. **Multi-Ember Fleet Architecture COMPLETE.** **Ember Survivability Hardening COMPLETE.** coralReef Deep Debt + Survivability + Fleet Evolution complete.

**Notes:** Phase 2 transition complete: Rust+Python baselines now validate NUCLEUS composition patterns via IPC. `niche.rs` declares hotSpring's proto-nucleate, capabilities, and primal dependencies. `composition.rs` probes atomic health. `hotspring_primal.rs` serves `health.*`, `capability.*`, `composition.*`, and `mcp.tools.list` JSON-RPC endpoints. `harvest-ecobin.sh` builds musl-static binaries for plasmidBin. SovereignInit Pipeline (Exp 164-165): 8-stage pure Rust nouveau replacement. Ember fleet: per-device isolation, hot-standby pool, fault-informed resurrection. **Fleet: Titan V (GV100) + 2× Tesla K80 (GK210) + RTX 5070 (GB206).** Chuna Papers 43-45: **44/44 overnight checks pass**. All AGPL-3.0-only.
...
- **toadStool** (`../../../phase1/toadStool/`): Hardware discovery and orchestration (S168+). PcieTransport, ResourceOrchestrator, GPU sysmon telemetry, `shader.dispatch`.
- **Experiment journals**: `../../experiments/` (001-165; 001-143 archived)
