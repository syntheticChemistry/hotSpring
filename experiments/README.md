...
| 179 | K80_WARM_FECS_DISPATCH_PIPELINE | investigation | ⚠️ **K80 warm-catch FECS/PFIFO pipeline.** Nouveau warm-catch → VFIO rebind. FECS boots (internal firmware, Falcon v3 PIO). PFIFO runlist completes (dynamic PBDMA→runlist discovery from 0x2390). GPFIFO dispatch blocked: SCHED_ERROR code=32, PBDMA stale state clearing added. |
