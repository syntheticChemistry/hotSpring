...
pub struct Stage {
    ...
    pub name: String,
    /// Kind of substrate this stage runs on.
    pub substrate_kind: SubstrateKind,
    /// Specific substrate name (from discovery), if pinned.
    pub substrate_name: Option<String>,
    /// What this stage does (compute, inference, validation, etc.).
    pub role: StageRole,
}
...
#[derive(Debug)]
pub struct Pipeline {
    /// Pipeline name for display and telemetry.
    pub name: String,
    stages: Vec<Stage>,
    edges: Vec<Edge>,
}
...
#[derive(Debug, Clone)]
pub struct WorkUnit {
    /// Monotonic work-unit identifier.
    pub id: u64,
    /// Current stage in the pipeline.
    pub stage_id: StageId,
    /// Named floating-point values (plaquette, delta_h, etc.).
...
pub struct PipelineResult {
    /// Name of the topology that was evaluated.
    ...
    /// Total energy consumed in joules (if RAPL/smi available).
