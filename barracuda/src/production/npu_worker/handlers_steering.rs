...
//! NPU handlers for thermalization, steering, and run planning.
//!
//! Covers: therm convergence, rejection prediction, phase classification,
//! quality scoring, anomaly detection, adaptive steering, next-run recommendation.
...
pub(super) fn handle_steer_adaptive(
    ...
    if saturated {
        eprintln!(
            "  [NPU] Saturation detected: gap={largest_gap:.3}/{range:.1} score={best_score:.3} n={n_measured} — recommend moving on"
        );
    }
