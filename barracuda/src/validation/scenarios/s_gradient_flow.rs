...
pub fn run(v: &mut ValidationHarness) {
    ...
    if measurements.len() >= 2 {
        ...
        let last = measurements.last().unwrap().energy_density;
        ...
        v.check_bool(
            "gradient-flow:cold_flow_stable",
            last.abs() < 1.0,
        );
    }
    ...
    v.check_bool("gradient-flow:hot_start_finite", hot_action_before.is_finite());

    if hot_measurements.len() >= 2 {
        let hot_first = hot_measurements[0].energy_density;
        let hot_last = hot_measurements.last().unwrap().energy_density;
