...
//
// Positions, velocities, and forces stay in ORIGINAL particle order on GPU.
// The cell-list provides sorted_indices that map sorted slots → original
// particle indices. This eliminates the need to sort/unsort N×3 f64 arrays
// on each cell-list rebuild.
//
// Compared to yukawa_force_celllist_f64.wgsl:
//   - Added binding 6: sorted_indices [N] u32
//   - Neighbor access: positions[sorted_indices[start + jj] * 3u]
//   - Thread i handles original particle i (not sorted position i)
//   - Forces/PE written to original index (direct, no unsort needed)
//
// Benefit: cell-list rebuild is O(N) GPU-only (3-pass bin/scan/scatter),
// no CPU readback of positions/velocities/forces.
//
// Bindings:
//   0: positions       [N*3]          f64, read  — ORIGINAL particle order
//   1: forces          [N*3]          f64, write
//   2: pe_buf          [N]            f64, write — per-particle PE (half-counted)
//   3: params          [16]           f64, read  — simulation parameters
//   4: cell_start      [n_cells_total] u32, read — first sorted slot in each cell
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Thread i handles original particle i
    let xi = positions[i * 3u];
    let yi = positions[i * 3u + 1u];
    let zi = positions[i * 3u + 2u];

    // Determine which cell particle i is in (from its coordinates)
        ...
        for (var dy = -1; dy <= 1; dy = dy + 1) {
            for (var dx = -1; dx <= 1; dx = dx + 1) {
                ...
                for (var jj = 0u; jj < count; jj = jj + 1u) {
                    // Indirect: map sorted slot → original particle index
