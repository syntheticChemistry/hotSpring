...
pub async fn run_simulation(
    ...
    // Select precision strategy based on GPU hardware:
    //   Native (Titan V, V100): native f64 on all shader ops
    //   Hybrid (RTX 3090, consumer): DF64 on FP32 cores, f64 for PBC/I/O
...
pub async fn run_simulation_verlet_with_brain(
    ...
    let max_neighbors = crate::tolerances::VERLET_MAX_NEIGHBORS;
    let verlet = crate::md::neighbor::VerletListGpu::new(
        &gpu,
        n,
        [box_side; 3],
        config.rc,
    ...
    println!(
        "    Verlet list: {}×{}×{} cells, skin={:.3}, max_nb={}",
        mx, my, mz, skin, max_neighbors
    );
    ...
    let equil_time = t_equil.elapsed().as_secs_f64();
    println!(
        "    Equilibration complete in {equil_time:.2}s ({rebuild_count} Verlet rebuilds)"
    );

    // ── Production ──
    println!("  ── Production ({} steps, Verlet adaptive rebuild) ──", config.prod_steps);
    let t_prod = Instant::now();
    let mut energy_history = Vec::new();
    let mut positions_snapshots = Vec::new();
    let mut velocity_snapshots = Vec::new();
