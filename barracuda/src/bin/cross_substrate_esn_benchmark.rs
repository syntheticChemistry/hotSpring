...
use hotspring_barracuda::md::reservoir::{EchoStateNetwork, EsnConfig, ExportedWeights, NpuSimulator};
use hotspring_barracuda::md::shaders::{SHADER_ESN_READOUT, SHADER_ESN_RESERVOIR_UPDATE};
...
impl GpuEsn {
    fn new(gpu: &GpuF64, weights: &ExportedWeights) -> Self {
    ...
    fn predict(&self, gpu: &GpuF64, input_sequence: &[Vec<f64>]) -> Vec<f64> {
    ...
    fn predict_batched(&self, gpu: &GpuF64, input_sequence: &[Vec<f64>]) -> Vec<f64> {
        let rs = self.reservoir_size;
        let wg = rs.div_ceil(64) as u32;
        ...
        // Readout only (no intermediate readback during sequence)
        let readout_bg = gpu.create_bind_group(
            &self.readout_pipeline,
            &[
                &self.w_out_buf,
...
fn generate_test_sequence(seed: u64, length: usize, input_size: usize) -> Vec<Vec<f64>> {
    let mut state = seed;
    (0..length)
        .map(|_| {
            (0..input_size)
...
fn main() {
    ...
    for &rs in &reservoir_sizes {
        ...
        let (gpu_mean, gpu_std, gpu_pred) = time_fn(
            || gpu_esn.predict(&gpu, &test_seq_gpu),
            N_WARMUP,
            N_REPS,
        );
        println!(
    ...
    println!("  3a. Threshold Detection (binary classifier)");
    {
        ...
        println!("    Accuracy: {correct}/{n_samples} = {:.1}%", accuracy * 100.0);
    ...
    println!("  3b. Streaming Inference Throughput");
    {
        ...
        harness.check_bool("streaming_npu_functional", npu_predictions.len() == stream_len);
        harness.check_bool("streaming_gpu_functional", gpu_predictions.len() == stream_len);
    ...
    for &rs in &large_sizes {
        ...
        let cpu_pred = esn.predict(&test_seq).unwrap_or_default();
        let test_seq_c = test_seq.clone();
        let (cpu_us, _, _) = time_fn(
            || esn.predict(&test_seq_c).unwrap_or_default(),
            3,
            10,
        );
        ...
        let gpu_esn = GpuEsn::new(&gpu, &exported);
        let test_seq_g = test_seq.clone();
        let (gpu_us, _, gpu_pred) = time_fn(
            || gpu_esn.predict_batched(&gpu, &test_seq_g),
            3,
            10,
        );
    ...
    // Task A: Thermalization detection across substrates
    {
        ...
        println!("    NPU-sim: {npu_correct}/{n_test} = {:.1}%", npu_acc * 100.0);
        println!("    GPU-ESN: {gpu_correct}/{n_test} = {:.1}%", gpu_acc * 100.0);
    ...
    // Write JSONL log
    let log_dir = "/tmp/hotspring-runs/exp021";
    std::fs::create_dir_all(log_dir).ok();
    let log_path = format!("{log_dir}/cross_substrate_results.jsonl");
...
fn run_cpu_only_experiments(harness: &mut ValidationHarness) {
    ...
    for &rs in &reservoir_sizes {
        ...
        println!("  RS={rs}: CPU={:.6}, NPU={:.6}, rel_diff={rel:.4e}", cpu_pred[0], npu_pred[0]);
