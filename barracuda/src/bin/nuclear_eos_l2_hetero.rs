...
use hotspring_barracuda::prescreen::{
    nmp_prescreen, l1_proxy_prescreen, NMPConstraints, NMPScreenResult,
    PreScreenClassifier, CascadeStats,
};
...
fn generate_l1_training_data(
    ...
    println!("  Phase 1: Generating L1 training data ({} samples)...", n_samples);
    let t0 = Instant::now();
    ...
    for sample in &samples {
        ...
        let nmp = match nuclear_matter_properties(&params) {
            Some(nmp) => nmp,
            None => { xs.push(params); ys.push(9.2); continue; },
        };

        let mut penalty = 0.0;
        if nmp.rho0_fm3 < 0.08 { penalty += 50.0 * (0.08 - nmp.rho0_fm3) / 0.08; }
        else if nmp.rho0_fm3 > 0.25 { penalty += 50.0 * (nmp.rho0_fm3 - 0.25) / 0.25; }
    ...
    println!("    Generated {} samples in {:.2}s", n_samples, t0.elapsed().as_secs_f64());
    println!("    Good (log(1+χ²) < 5): {} ({:.1}%)", n_good, 100.0 * n_good as f64 / n_samples as f64);
    println!();
...
fn train_classifier(xs: &[Vec<f64>], ys: &[f64]) -> ClassifierResult {
    ...
    for (x, &y) in xs.iter().zip(ys.iter()) {
        ...
        if pred && actual { tp += 1; }
        if pred && !actual { fp += 1; }
        if !pred && actual { fn_ += 1; }
        if !pred && !actual { tn += 1; }
    }

    let n_positive = tp + fn_;
    let recall = if n_positive > 0 { tp as f64 / n_positive as f64 } else { 0.0 };
    let precision = if tp + fp > 0 { tp as f64 / (tp + fp) as f64 } else { 0.0 };
    let accuracy = (tp + tn) as f64 / xs.len().max(1) as f64;

    println!("    Trained in {:.3}s on {} samples ({} positive at threshold={:.1})", 
        t0.elapsed().as_secs_f64(), xs.len(), n_positive, label_threshold);
    ...
    let usable = if recall < 0.5 && n_positive > 5 {
        ...
        for t in (5..50).map(|i| i as f64 / 100.0) {
            ...
            for (x, &y) in xs.iter().zip(ys.iter()) {
                let pred = classifier.predict(x);
                let actual = y < label_threshold;
                if pred && actual { tp2 += 1; }
                if !pred && actual { fn2 += 1; }
            }
            let r = if tp2 + fn2 > 0 { tp2 as f64 / (tp2 + fn2) as f64 } else { 0.0 };
            if r > best_recall {
                best_recall = r;
                best_threshold = t;
            }
        ...
        println!("    ⚠ Low recall — adjusted threshold to {:.2} (recall now {:.1}%)", 
            best_threshold, best_recall * 100.0);
        best_recall >= 0.1  // usable if at least 10% recall after adjustment
    } else if n_positive <= 5 {
        println!("    ⚠ Too few positives ({}) — classifier DISABLED (bypass Tier 3)", n_positive);
        false
    } else {
        true
    };
    ...
    ClassifierResult { classifier, recall, usable }
}
...
fn run_heterogeneous_l2(
    ...
    for seed in &seed_params {
        // Generate 20 perturbations around each seed (within 10% of range)
        for _ in 0..20 {
            let mut candidate = seed.to_vec();
            for (i, &(lo, hi)) in bounds.iter().enumerate() {
                let range = hi - lo;
                rng_state = rng_state.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
                let u = (rng_state >> 33) as f64 / (1u64 << 31) as f64; // [0, 1)
                let perturbation = (u - 0.5) * 0.2 * range; // ±10% of range
    ...
    let lhs_samples = latin_hypercube(500, bounds, 777)
        .expect("LHS sampling failed");
    for sample in &lhs_samples {
        initial_candidates.push(sample.clone());
    }

    println!("    Initial pool: {} candidates ({} seeded + {} LHS)",
        initial_candidates.len(), seed_params.len() * 20, lhs_samples.len());
    ...
    for params in &initial_candidates {
        ...
        match nmp_prescreen(params, &constraints) {
            NMPScreenResult::Fail(_) => { stats.tier1_rejected += 1; continue; },
    ...
    println!("    Survived cascade: {} / {} ({:.1}%)",
        surviving_candidates.len(), initial_candidates.len(),
        100.0 * surviving_candidates.len() as f64 / initial_candidates.len() as f64);

    // ── Evaluate survivors with HFB (expensive, parallel) ──
    println!("    Evaluating {} survivors with HFB (parallel)...", surviving_candidates.len());
    ...
    let init_best = eval_results.iter().map(|(_, f)| *f).fold(f64::INFINITY, f64::min);
    println!("    Initial evaluations: {}, best log(1+χ²) = {:.4}", n_initial, init_best);
    ...
    println!("    Surrogate refinement ({} rounds):", n_rounds);
    ...
    for round in 0..n_rounds {
        ...
        if train_x.len() < 5 {
            println!("      Round {}: too few data points ({}), skipping", round + 1, train_x.len());
            continue;
        }

        // Build RBF surrogate on real HFB values (no penalties!)
        let surrogate = match RBFSurrogate::train(&train_x, &train_y, RBFKernel::ThinPlateSpline, 0.0) {
            Ok(s) => s,
            Err(e) => {
                println!("      Round {}: surrogate training failed: {:?}", round + 1, e);
                continue;
            }
        };
        ...
        let surrogate_fn = |x: &[f64]| -> f64 {
            // Bounds check
            if x.iter().zip(bounds.iter()).any(|(&v, &(lo, hi))| v < lo || v > hi) {
                return 1e10;
            }
        ...
        for _ in 0..candidates_per_round / 4 {
            let mut candidate = nm_result.x_best.clone();
            for (i, &(lo, hi)) in bounds.iter().enumerate() {
                let range = hi - lo;
                rng_state = rng_state.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
                let u = (rng_state >> 33) as f64 / (1u64 << 31) as f64;
                let perturbation = (u - 0.5) * 0.1 * range;
                candidate[i] = (candidate[i] + perturbation).clamp(lo, hi);
            }
        ...
        // Best-so-far point + perturbations (exploit known good region)
        {
            ...
            if let Some(best_rec) = cache.best() {
        ...
        let mut round_survivors: Vec<Vec<f64>> = Vec::new();
        for params in &round_candidates {
            let mut stats = cascade_stats.lock().unwrap();
            stats.total_candidates += 1;
            ...
            round_survivors.push(params.clone());
        }
    ...
    if cache.is_empty() {
        ...
        for i in 1..l1_ys.len() {
            if l1_ys[i] < l1_ys[best_idx] { best_idx = i; }
        }
        return (l1_xs[best_idx].clone(), l1_ys[best_idx], cache.clone(), final_stats);
    }
...
fn l2_objective(params: &[f64], nuclei: &[(usize, usize, f64)]) -> f64 {
    ...
    let results: Vec<(f64, f64)> = nuclei.iter()
        .map(|&(z, n, b_exp)| {
...
fn run_plain_l2(
    ...
    println!("    χ²/datum: {:.2}, time: {:.1}s, evals: {}", chi2, elapsed.as_secs_f64(), result.cache.len());
    ...
    match mode {
        "both" | "compare" => {
            ...
            let (best_params, best_f, cache, stats) = run_heterogeneous_l2(
                &bounds, &exp_data, &l1_xs, &l1_ys, &clf_result, n_rounds, candidates_per_round
            );
            ...
            println!("║    χ²/datum = {:12.2}                                  ║", hetero_chi2);
            println!("║    Time:      {:6.1}s                                      ║", hetero_time);
            println!("║    HFB evals: {:6}                                        ║", cache.len());
            println!("║    Cascade pass rate: {:.1}%                                ║", stats.pass_rate() * 100.0);
            println!("╠══════════════════════════════════════════════════════════════╣");
            println!("║  Plain SparsitySampler:                                    ║");
            println!("║    χ²/datum = {:12.2}                                  ║", plain_chi2);
            println!("║    Time:      {:6.1}s                                      ║", plain_time);
            println!("║    HFB evals: {:6}                                        ║", plain_evals);
            println!("╚══════════════════════════════════════════════════════════════╝");
        ...
        _ => {
            ...
            println!("║  log(1+χ²):      {:12.4}                              ║", best_f);
            println!("║  HFB evals:      {:6}                                    ║", cache.len());
            println!("║  Total time:     {:6.1}s                                   ║", total_time);
            println!("║  HFB throughput: {:6.1} evals/s                            ║",
            ...
            save_results(&base, &best_params, best_f, &cache, &stats, total_time, None, None, None);
        }
    }
}
