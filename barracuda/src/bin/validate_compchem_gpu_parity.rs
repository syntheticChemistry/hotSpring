...
fn main() {
    ...
    let modules_1d = [
        ("free_xylose_1d", "HILLS"),
        ("enzyme_bound_1d", "HILLS"),
    ];
    ...
    for (sys, hills_name) in &modules_1d {
        ...
        if !hills_path.exists() {
            let gs_path = gs_dir.join("modules").join(format!("0{}_{}", if *sys == "free_xylose_1d" { 3 } else { 5 }, sys)).join(hills_name);
            if gs_path.exists() {
    ...
    for (sys, hills_name) in &modules_2d {
        ...
        if !hills_path.exists() {
            ...
            let gs_path = gs_dir.join("modules").join(format!("0{}_{}", mod_num, sys)).join(hills_name);
            if gs_path.exists() {
...
fn run_1d_parity(hills_path: &Path, name: &str, gpu: &Option<GpuF64>, harness: &mut ValidationHarness) {
    ...
    if gpu.is_some() {
        ...
        println!("    {} {} — RMSD {:.2e} kJ/mol (CPU {:.1}ms, GPU {:.1}ms) {}",
            if pass { "\x1b[32m✓\x1b[0m" } else { "\x1b[31m✗\x1b[0m" },
            name, rmsd, cpu_time.as_secs_f64() * 1000.0, gpu_time.as_secs_f64() * 1000.0,
            if pass { "" } else { "DIVERGENCE" });
        harness.check_bool(&format!("1d:{}:gpu_parity", name), pass);
    } else {
        println!("    [CPU] {} — {} bins, {} Gaussians ({:.1}ms)",
            name, nbins, hills.n_gaussians, cpu_time.as_secs_f64() * 1000.0);
        harness.check_bool(&format!("1d:{}:cpu_reconstructed", name), true);
    }
}

fn run_2d_parity(hills_path: &Path, name: &str, gpu: &Option<GpuF64>, harness: &mut ValidationHarness, fes_pipeline: &Option<FesGaussianSumGpu>) {
    ...
    let grid_min_x = hills.centers_x.iter().cloned().fold(f64::INFINITY, f64::min) - margin_x;
    let grid_max_x = hills.centers_x.iter().cloned().fold(f64::NEG_INFINITY, f64::max) + margin_x;
    let grid_min_y = hills.centers_y.iter().cloned().fold(f64::INFINITY, f64::min) - margin_y;
    let grid_max_y = hills.centers_y.iter().cloned().fold(f64::NEG_INFINITY, f64::max) + margin_y;

    let cpu_start = Instant::now();
    let cpu_fes = reconstruct_2d_cpu(&hills, grid_min_x, grid_max_x, grid_min_y, grid_max_y, nbins);
    let cpu_time = cpu_start.elapsed();

    if let Some(pipeline) = fes_pipeline {
        let hills_packed: Vec<f64> = (0..hills.n_gaussians)
            .flat_map(|g| vec![
                hills.centers_x[g], hills.centers_y[g],
                hills.sigmas_x[g], hills.sigmas_y[g],
                hills.heights[g],
            ])
            .collect();

        let result = pipeline.reconstruct_2d(
            &hills_packed, hills.n_gaussians,
        ...
        println!("    {} {} — RMSD {:.2e} kJ/mol (CPU {:.1}ms, GPU {:.1}ms) {}",
            if pass { "\x1b[32m✓\x1b[0m" } else { "\x1b[31m✗\x1b[0m" },
            name, rmsd, cpu_time.as_secs_f64() * 1000.0, result.gpu_secs * 1000.0,
            if pass { "" } else { "DIVERGENCE" });
        harness.check_bool(&format!("2d:{}:gpu_parity", name), pass);
    } else {
        println!("    [CPU] {} — {}×{} bins, {} Gaussians ({:.1}ms)",
            name, nbins, nbins, hills.n_gaussians, cpu_time.as_secs_f64() * 1000.0);
...
fn parse_hills_1d(path: &Path) -> Option<Hills1D> {
    ...
    let n = centers.len();
    if n == 0 { return None; }
    Some(Hills1D { centers, sigmas, heights, n_gaussians: n })
}
...
fn parse_hills_2d(path: &Path) -> Option<Hills2D> {
    ...
    let n = cx.len();
    if n == 0 { return None; }
    Some(Hills2D { centers_x: cx, centers_y: cy, sigmas_x: sx, sigmas_y: sy, heights, n_gaussians: n })
}
...
fn reconstruct_1d_cpu(hills: &Hills1D, grid_min: f64, grid_max: f64, nbins: usize) -> Vec<f64> {
    ...
    for f in &mut fes { *f -= min_val; }
    fes
}

fn reconstruct_2d_cpu(hills: &Hills2D, gmin_x: f64, gmax_x: f64, gmin_y: f64, gmax_y: f64, nbins: usize) -> Vec<f64> {
...
fn reconstruct_1d_gpu_emulated(hills: &Hills1D, grid_min: f64, grid_max: f64, nbins: usize) -> Vec<f64> {
    use std::sync::Arc;
    use std::thread;
...
fn reconstruct_2d_gpu_emulated(hills: &Hills2D, gmin_x: f64, gmax_x: f64, gmin_y: f64, gmax_y: f64, nbins: usize) -> Vec<f64> {
    use std::sync::Arc;
    use std::thread;
...
fn compute_rmsd_1d(a: &[f64], b: &[f64]) -> f64 {
    ...
    let sum_sq: f64 = a[..n].iter().zip(b[..n].iter())
        .map(|(x, y)| (x - y) * (x - y))
        .sum();
...
fn find_spring_root() -> PathBuf {
    ...
    loop {
        if dir.join("pseudoSpore_hotSpring-CompChem-GuideStone_v1.5.0").exists() {
