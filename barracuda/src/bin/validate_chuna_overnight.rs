...
/// generated by MILC. This section thermalizes with `dynamical_hmc_trajectory`
/// (staggered pseudofermion HMC) and then runs W7 gradient flow, validating
/// that scale setting (t₀, w₀) works on dynamical backgrounds — not just
/// quenched.
fn paper_43_dynamical(harness: &mut ValidationHarness) {
    ...
    let start = Instant::now();
    println!("  Dynamical 8⁴ β=5.4 (N_f=4 staggered, m=0.1)...");
    ...
    let mut config = DynamicalHmcConfig {
        ...
        beta,
        n_flavors_over_4: 1, // N_f = 4 staggered flavors
        ..Default::default()
    };
    ...
    println!(
        "    ⟨P⟩ = {plaq:.6}, {:.0}% accept, {total_cg} CG iters",
        acceptance * 100.0
    );

    harness.check_lower("p43_dyn_accept", acceptance, 0.20);
