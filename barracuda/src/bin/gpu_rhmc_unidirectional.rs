...
//!
//! Same self-tuning physics as `gpu_rhmc_selftuning`, but using the
//! unidirectional pipeline: GPU-resident shifted CG, GPU-resident
//! Hamiltonian reduces, GPU PRNG. Eliminates ~19,000 CPU-GPU syncs
//! per trajectory, replacing them with ~100 × 8-byte convergence checks.
//!
//! Usage:
//!   cargo run --release --bin gpu_rhmc_unidirectional -- \
...
fn main() {
    ...
    println!("═══════════════════════════════════════════════════════════════");
    println!("  Unidirectional GPU RHMC — Zero Per-Iteration Readback");
    println!("═══════════════════════════════════════════════════════════════");
    println!(
    ...
    println!("  Configs: {n_configs}  |  Pipeline: unidirectional (GPU-resident CG + reduce)");
    println!();
    ...
    // ═══ Pipeline compilation (includes unidirectional pipelines) ═══
    let t0 = Instant::now();
    ...
    // ═══ RHMC physics (spectral + rational approx) ═════════════════
    println!("\n─── RHMC physics initialization ───");
    ...
    // ═══ Allocate unidirectional buffers ════════════════════════════
    ...
    // ═══ dt discovery (using old sync path — only a few probes) ════
    println!("\n─── dt discovery ───");
    ...
    // ═══ RHMC thermalization (UNIDIRECTIONAL pipeline) ══════════════
    println!("\n─── Thermalization (UNIDIRECTIONAL — n_md=1, dt adapts) ───");
    ...
    for i in 1..=therm_max {
        ...
        if i % 10 == 0 || i <= 3 {
            println!(
                "  therm {:>3}: P={:.6} ΔH={:>8.3} {} acc={:.0}% dt={:.2e} cg={} ({:.2}s)",
                i,
                r.plaquette,
                r.delta_h,
                if r.accepted { "✓" } else { "✗" },
    ...
    let therm_secs = t3.elapsed().as_secs_f64();
    println!(
        "  Thermalization: {therm_secs:.1}s, dt={:.2e}, acc={:.0}%",
        ctrl.dt,
        ctrl.acceptance_rate() * 100.0
    );
    ...
    println!("  pipeline  = unidirectional (GPU-resident CG + reduce)");
