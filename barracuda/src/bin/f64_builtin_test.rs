...
fn patch_math_f64_preamble(preamble: &str) -> String {
    ...
    for line in preamble.lines() {
        ...
        let line = line.replace("1e308", "1e38").replace("-1e308", "-1e38");
...
"#, body);
        ...
        match err {
            Some(e) => println!("  {:<15} FAILED — {}", name, e),
            None => println!("  {:<15} COMPILED OK", name),
        }
    }
    ...
    let n = 1024u32;
    let test_values: Vec<f64> = (0..n).map(|i| (i as f64 + 1.0) * 0.01).collect();
...
"#;
    ...
    let math_preamble = patch_math_f64_preamble(
        &ShaderTemplate::math_f64_preamble()
    );
    let sw_sqrt_shader = format!(r#"
{}
...
"#, math_preamble);
    ...
    // Run native sqrt
    let native_results = run_shader(&gpu, native_sqrt_shader, &input_buf, &output_buf, &staging_buf, n).await;
    // Run software sqrt
    let sw_results = run_shader(&gpu, &sw_sqrt_shader, &input_buf, &output_buf, &staging_buf, n).await;
    ...
    for i in 0..n as usize {
        ...
        let diff_sw = (sw_results[i] - cpu_val).abs();
    ...
    println!("  Native vs software max diff: {:.2e} (at index {})", max_diff, max_diff_idx);
    println!("  Native vs CPU f64:           {:.2e}", max_diff_cpu);
    println!("  Sample: sqrt(2.0): native={:.15}, sw={:.15}, cpu={:.15}",
        native_results[199], sw_results[199], test_values[199].sqrt());
...
"#, math_preamble);
    ...
    let exp_values: Vec<f64> = (0..n).map(|i| -(i as f64 + 1.0) * 0.01).collect();
    let exp_input_bytes: Vec<u8> = exp_values.iter().flat_map(|v| v.to_le_bytes()).collect();
    let exp_input_buf = gpu.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        label: Some("exp_input"),
        contents: &exp_input_bytes,
        usage: wgpu::BufferUsages::STORAGE,
    });

    let native_exp = run_shader(&gpu, native_exp_shader, &exp_input_buf, &output_buf, &staging_buf, n).await;
    let sw_exp = run_shader(&gpu, &sw_exp_shader, &exp_input_buf, &output_buf, &staging_buf, n).await;
    ...
    for i in 0..n as usize {
        ...
        if diff_nat_sw > max_exp_diff { max_exp_diff = diff_nat_sw; }
        if diff_cpu > max_exp_diff_cpu { max_exp_diff_cpu = diff_cpu; }
    }
    println!("  Native vs software max diff: {:.2e}", max_exp_diff);
    println!("  Native vs CPU f64:           {:.2e}", max_exp_diff_cpu);
    println!("  Sample: exp(-2.0): native={:.15}, sw={:.15}, cpu={:.15}",
        native_exp[199], sw_exp[199], exp_values[199].exp());
    ...
    let big_values: Vec<f64> = (0..big_n).map(|i| (i as f64 + 1.0) * 0.000001).collect();
    let big_bytes: Vec<u8> = big_values.iter().flat_map(|v| v.to_le_bytes()).collect();
    let big_input = gpu.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
    ...
    let t0 = Instant::now();
    for _ in 0..iters {
        let _ = run_shader(&gpu, native_sqrt_shader, &big_input, &big_output, &big_staging, big_n).await;
    }
    let native_sqrt_time = t0.elapsed().as_secs_f64() / iters as f64;

    let t0 = Instant::now();
    for _ in 0..iters {
        let _ = run_shader(&gpu, &sw_sqrt_shader, &big_input, &big_output, &big_staging, big_n).await;
    }
    let sw_sqrt_time = t0.elapsed().as_secs_f64() / iters as f64;

    let t0 = Instant::now();
    for _ in 0..iters {
        let _ = run_shader(&gpu, native_exp_shader, &big_input, &big_output, &big_staging, big_n).await;
    }
    let native_exp_time = t0.elapsed().as_secs_f64() / iters as f64;

    let t0 = Instant::now();
    for _ in 0..iters {
        let _ = run_shader(&gpu, &sw_exp_shader, &big_input, &big_output, &big_staging, big_n).await;
    }
    let sw_exp_time = t0.elapsed().as_secs_f64() / iters as f64;

    println!("  {:>20} {:>12} {:>12} {:>10}", "Function", "Native", "Software", "Speedup");
    println!("  {:>20} {:>12} {:>12} {:>10}", "────────", "──────", "────────", "───────");
    println!("  {:>20} {:>10.3}ms {:>10.3}ms {:>9.1}×",
        "sqrt (1M f64)", native_sqrt_time * 1000.0, sw_sqrt_time * 1000.0,
        sw_sqrt_time / native_sqrt_time);
    println!("  {:>20} {:>10.3}ms {:>10.3}ms {:>9.1}×",
        "exp (1M f64)", native_exp_time * 1000.0, sw_exp_time * 1000.0,
...
async fn run_shader(
    ...
    encoder.copy_buffer_to_buffer(output_buf, 0, staging_buf, 0, (n as u64) * 8);
    gpu.queue.submit(Some(encoder.finish()));

    let slice = staging_buf.slice(..);
    let (tx, rx) = std::sync::mpsc::channel();
    slice.map_async(wgpu::MapMode::Read, move |r: Result<(), wgpu::BufferAsyncError>| { tx.send(r).ok(); });
    gpu.device.poll(wgpu::Maintain::Wait);
    rx.recv().unwrap().unwrap();

    let data = slice.get_mapped_range();
    let result: Vec<f64> = data.chunks_exact(8)
