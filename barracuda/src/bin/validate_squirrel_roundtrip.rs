...
fn main() {
    ...
    let squirrel_alive = ctx.get_by_capability("inference").is_some_and(|ep| ep.alive);

    result.section("Squirrel Discovery");
    if squirrel_alive {
        result.check_bool("squirrel_discovered", true, "Squirrel found via capability routing");
    } else {
        result.check_skip(
            "squirrel_discovered",
    ...
    result.check_or_skip("models_list", squirrel_alive.then_some(&ctx), "no Squirrel", |ctx, cr| {
        match squirrel_client::inference_models(ctx) {
            Ok(models) => {
    ...
    result.check_or_skip(
        ...
        "no Squirrel",
        |ctx, cr| {
            match squirrel_client::inference_complete(ctx, "What is lattice QCD?", None) {
                Ok(text) => {
    ...
    result.check_or_skip(
        ...
        "no Squirrel",
        |ctx, cr| {
            match squirrel_client::inference_embed(ctx, "lattice QCD Monte Carlo") {
                Ok(embedding) => {
                    let ok = !embedding.is_empty()
